...
local npc_REPLACEME_NEXTBOTNAME_acquire_distance = CreateConVar("npc_REPLACEME_NEXTBOTNAME_acquire_distance", 2500, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                "The maximum distance at which REPLACEME_NEXTBOTNAME will chase a target.")

local npc_REPLACEME_NEXTBOTNAME_spawn_protect = CreateConVar("npc_REPLACEME_NEXTBOTNAME_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, REPLACEME_NEXTBOTNAME will not target players or hide within 200 units of a spawn point.")

local npc_REPLACEME_NEXTBOTNAME_attack_distance = CreateConVar("npc_REPLACEME_NEXTBOTNAME_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of REPLACEME_NEXTBOTNAME's attack.")

local npc_REPLACEME_NEXTBOTNAME_attack_interval = CreateConVar("npc_REPLACEME_NEXTBOTNAME_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between REPLACEME_NEXTBOTNAME's attacks.")

local npc_REPLACEME_NEXTBOTNAME_attack_force = CreateConVar("npc_REPLACEME_NEXTBOTNAME_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of REPLACEME_NEXTBOTNAME's attack. Higher values throw things farther.")

local npc_REPLACEME_NEXTBOTNAME_smash_props = CreateConVar("npc_REPLACEME_NEXTBOTNAME_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, REPLACEME_NEXTBOTNAME will punch through any props placed in their way.")

local npc_REPLACEME_NEXTBOTNAME_hiding_scan_interval = CreateConVar("npc_REPLACEME_NEXTBOTNAME_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "REPLACEME_NEXTBOTNAME will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant REPLACEME_NEXTBOTNAMEs are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local npc_REPLACEME_NEXTBOTNAME_hiding_repath_interval = CreateConVar("npc_REPLACEME_NEXTBOTNAME_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to REPLACEME_NEXTBOTNAME's hiding spot will be redetermined every X seconds.")

local npc_REPLACEME_NEXTBOTNAME_chase_repath_interval = CreateConVar("npc_REPLACEME_NEXTBOTNAME_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of REPLACEME_NEXTBOTNAME's target will be redetermined every X seconds.")

local npc_REPLACEME_NEXTBOTNAME_expensive_scan_interval = CreateConVar("npc_REPLACEME_NEXTBOTNAME_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local npc_REPLACEME_NEXTBOTNAME_force_download = CreateConVar("npc_REPLACEME_NEXTBOTNAME_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download REPLACEME_NEXTBOTNAME resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear REPLACEME_NEXTBOTNAME!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

util.AddNetworkString("REPLACEME_NEXTBOTNAME_nag")
util.AddNetworkString("REPLACEME_NEXTBOTNAME_nagresponse")
util.AddNetworkString("REPLACEME_NEXTBOTNAME_navgen")
...
local VECTOR_REPLACEME_NEXTBOTNAME_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_REPLACEME_NEXTBOTNAME_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_REPLACEME_NEXTBOTNAME: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other REPLACEME_NEXTBOTNAMEs, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC() and
	        ent:Health() > 0 and
	        class ~= "npc_REPLACEME_NEXTBOTNAME" and
	        not class:find("bullseye"))
end

--HACK!!! Because this is an issue a lot of people keep asking me about.
hook.Add("PlayerSpawnedNPC", "REPLACEME_NEXTBOTNAMEMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_REPLACEME_NEXTBOTNAME") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.REPLACEME_NEXTBOTNAME_HasBeenNagged) then return end
	ply.REPLACEME_NEXTBOTNAME_HasBeenNagged = true

	-- Try to explain why REPLACEME_NEXTBOTNAME isn't working.
	net.Start("REPLACEME_NEXTBOTNAME_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("npc_REPLACEME_NEXTBOTNAME: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_REPLACEME_NEXTBOTNAME: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("REPLACEME_NEXTBOTNAME_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.REPLACEME_NEXTBOTNAME_HasBeenNagged = false
		return
	end
	...
	if (not IsValid(spawnPoint)) then
		net.Start("REPLACEME_NEXTBOTNAME_navgen")
			net.WriteBit(false)
		net.Send(ply)
	...
	--for _, REPLACEME_NEXTBOTNAME in pairs(ents.FindByClass("npc_REPLACEME_NEXTBOTNAME")) do
	--	REPLACEME_NEXTBOTNAME:Remove()
	--end

	print("npc_REPLACEME_NEXTBOTNAME: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "REPLACEME_NEXTBOTNAMENavGen", navEndGenerate)
	else
		print("npc_REPLACEME_NEXTBOTNAME: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_REPLACEME_NEXTBOTNAME_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `npc_REPLACEME_NEXTBOTNAME_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (npc_REPLACEME_NEXTBOTNAME_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_REPLACEME_NEXTBOTNAME_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. REPLACEME_NEXTBOTNAME means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * npc_REPLACEME_NEXTBOTNAME_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * npc_REPLACEME_NEXTBOTNAME_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not npc_REPLACEME_NEXTBOTNAME_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_REPLACEME_NEXTBOTNAME_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_REPLACEME_NEXTBOTNAME_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > npc_REPLACEME_NEXTBOTNAME_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > npc_REPLACEME_NEXTBOTNAME_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(npc_REPLACEME_NEXTBOTNAME_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > npc_REPLACEME_NEXTBOTNAME_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= npc_REPLACEME_NEXTBOTNAME_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= npc_REPLACEME_NEXTBOTNAME_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= npc_REPLACEME_NEXTBOTNAME_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= npc_REPLACEME_NEXTBOTNAME_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local REPLACEME_NEXTBOTNAMEsLastFrame = 0
hook.Add("Tick", "REPLACEME_NEXTBOTNAMEBenchmark", function()
	local now = SysTime()
	local numREPLACEME_NEXTBOTNAMEs = #ents.FindByClass("npc_REPLACEME_NEXTBOTNAME")

	if (timeAll ~= 0) then

		local REPLACEME_NEXTBOTNAMETime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    REPLACEME_NEXTBOTNAMETime / REPLACEME_NEXTBOTNAMEsLastFrame, REPLACEME_NEXTBOTNAMETime, frameTime, (REPLACEME_NEXTBOTNAMETime / frameTime) * 100, REPLACEME_NEXTBOTNAMEsLastFrame, numThink, (numThink / REPLACEME_NEXTBOTNAMEsLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	REPLACEME_NEXTBOTNAMEsLastFrame = numREPLACEME_NEXTBOTNAMEs
end)]]
...
language.Add("npc_REPLACEME_NEXTBOTNAME")
...
local function DevPrint(devLevel, msg)
	if (developer:GetInt() >= devLevel) then
		print("npc_REPLACEME_NEXTBOTNAME: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a REPLACEME_NEXTBOTNAME.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_REPLACEME_NEXTBOTNAME_music_volume = CreateConVar("npc_REPLACEME_NEXTBOTNAME_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by REPLACEME_NEXTBOTNAME. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another REPLACEME_NEXTBOTNAME comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, REPLACEME_NEXTBOTNAMEs do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_REPLACEME_NEXTBOTNAME_PANIC_COUNT REPLACEME_NEXTBOTNAMEs are this close, or an equivalent score.
local MUSIC_REPLACEME_NEXTBOTNAME_PANIC_COUNT = 8 -- That's a lot of REPLACEME_NEXTBOTNAME.

local MUSIC_REPLACEME_NEXTBOTNAME_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_REPLACEME_NEXTBOTNAME_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("npc_REPLACEME_NEXTBOTNAME") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("REPLACEME_NEXTBOTNAMEPanicMusicUpdate")
	...
	if (npc_REPLACEME_NEXTBOTNAME_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		panicMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "npc_REPLACEME_NEXTBOTNAME") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_REPLACEME_NEXTBOTNAME_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear REPLACEME_NEXTBOTNAME taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(npc_REPLACEME_NEXTBOTNAME_music_volume:GetFloat(), 0, 1))
...
local function startTimer()
	if (not timer.Exists("REPLACEME_NEXTBOTNAMEPanicMusicUpdate")) then
		timer.Create("REPLACEME_NEXTBOTNAMEPanicMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local REPLACEME_NEXTBOTNAMEMaterial = Material("REPLACEME_NEXTBOTNAME/npc_REPLACEME_NEXTBOTNAME.png")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(REPLACEME_NEXTBOTNAMEMaterial)
...
hook.Add("OnEntityCreated", "REPLACEME_NEXTBOTNAMEInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_REPLACEME_NEXTBOTNAME") then return end

	local REPLACEME_NEXTBOTNAMEEntTable = scripted_ents.GetStored("npc_REPLACEME_NEXTBOTNAME")

	table.Merge(ent, REPLACEME_NEXTBOTNAMEEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("REPLACEME_NEXTBOTNAME_removed", REPLACEME_NEXTBOTNAMEDeregister)
end)

hook.Add("NetworkEntityCreated", "REPLACEME_NEXTBOTNAMENetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_REPLACEME_NEXTBOTNAME") then return end
...
surface.CreateFont("REPLACEME_NEXTBOTNAMEHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("REPLACEME_NEXTBOTNAMEHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("REPLACEME_NEXTBOTNAME is studying this map.", "REPLACEME_NEXTBOTNAMEHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "REPLACEME_NEXTBOTNAMEHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "REPLACEME_NEXTBOTNAMEHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "REPLACEME_NEXTBOTNAMEHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "REPLACEME_NEXTBOTNAMEHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("REPLACEME_NEXTBOTNAME_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "REPLACEME_NEXTBOTNAMENavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. REPLACEME_NEXTBOTNAME doesn't even know where to start with this map.
...
local function navGenerate()
	net.Start("REPLACEME_NEXTBOTNAME_nagresponse")
		net.WriteBit(true)
	net.SendToServer()
end
...
local function navWarning()
	Derma_Query([[It will take a while for REPLACEME_NEXTBOTNAME to figure this map out.
...
-- Lazy
net.Receive("REPLACEME_NEXTBOTNAME_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! REPLACEME_NEXTBOTNAME doesn't know this map.
Would you like REPLACEME_NEXTBOTNAME to learn it?]],
		            "This map is currently not REPLACEME_NEXTBOTNAME-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! REPLACEME_NEXTBOTNAME doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to REPLACEME_NEXTBOTNAME.]],
		            "This map is currently not REPLACEME_NEXTBOTNAME-compatible!",
		            "Ok", nagAgain,
...
--
list.Set("NPC", "npc_REPLACEME_NEXTBOTNAME", {
	Name = "REPLACEME_NEXTBOTDISPLAYNAME",
	Class = "npc_REPLACEME_NEXTBOTNAME",
	Category = "[Livelandr's Generator] Nextbots",
	AdminOnly = true
})
